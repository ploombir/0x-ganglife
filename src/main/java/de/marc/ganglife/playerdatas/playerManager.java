...
public class playerManager extends QueryFactory {
    ...
    public CompletableFuture<Boolean> savePlayer(UPlayer uPlayer) {
        return builder().query("UPDATE accounts SET deathTime = ?, isJail = ?, cash = ?, phoneNumber = ?, phoneFlightMode = ?," +
                        ...
                        " WHERE uniqueId = ?")
                .parameter(stmt -> stmt.setInt(uPlayer.getDeathTime()).setBoolean(uPlayer.isJail()).setInt(uPlayer.getCash()).setInt(uPlayer.getPhoneNumber()).setBoolean(uPlayer.isPhoneFlightMode()).setString(uPlayer.getPhoneContacts()).setString(uPlayer.getPlayerName()).setInt(uPlayer.getBank()).setInt(uPlayer.getLevel()).setInt(uPlayer.getLevelExp()).setString(uPlayer.getFaction()).setInt(uPlayer.getFactionRank()).setInt(uPlayer.getDrink())
                        .setInt(uPlayer.getHouseNumber()).setString(uPlayer.getHouseInventory()).setInt(uPlayer.getPaydayTime()).setInt(uPlayer.getPlayTime()).setBoolean(uPlayer.isWeaponLicence()).setBoolean(uPlayer.isDriveLicence()).setBoolean(uPlayer.isFirstaidLicence()).setInt(uPlayer.getJailTime()).setString(uPlayer.getActReasons())
