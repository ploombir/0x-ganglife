...
public class setGarbageLevel extends QueryFactory {

    public setGarbageLevel(DataSource dataSource) {
        super(dataSource);
    }

    public CompletableFuture<Optional<Integer>> getExpSystem(UUID uniqueId) {
        return builder(Integer.class)
                .query("SELECT garbage_exp FROM accounts WHERE uniqueid = ?")
                .parameter(stmt -> stmt.setString(uniqueId.toString()))
                .readRow(row -> row.getInt("garbage_exp"))
                .first();
    }

    public CompletableFuture<Boolean> addExpSystem(UUID uniqueId, int amount) {
        return builder().query("UPDATE accounts SET garbage_exp = garbage_exp + ? WHERE uniqueid = ?")
    ...
    public CompletableFuture<Boolean> setExpSystem(UUID uniqueId, int amount) {
        return builder().query("UPDATE accounts SET garbage_exp = ? WHERE uniqueid = ?")
                .parameter(stmt -> stmt.setInt(amount)
                        .setString(uniqueId.toString()))
                .update()
                .send()
    ...
    public CompletableFuture<Optional<Integer>> getLevelSystem(UUID uniqueId) {
        return builder(Integer.class)
                .query("SELECT garbage_level FROM accounts WHERE uniqueid = ?")
                .parameter(stmt -> stmt.setString(uniqueId.toString()))
                .readRow(row -> row.getInt("garbage_level"))
                .first();
    }

    public CompletableFuture<Boolean> addLevelSystem(UUID uniqueId, int amount) {
        return builder().query("UPDATE accounts SET garbage_level = garbage_level + ? WHERE uniqueid = ?")
                .parameter(stmt -> stmt.setInt(amount)
                        .setString(uniqueId.toString()))
                .update()
                .send()
    ...
    public CompletableFuture<Boolean> setLevelSystem(UUID uniqueId, int amount) {
        return builder().query("UPDATE accounts SET garbage_level = ? WHERE uniqueid = ?")
                .parameter(stmt -> stmt.setInt(amount)
                        .setString(uniqueId.toString()))
                .update()
                .send()
